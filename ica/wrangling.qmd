...
```{r eval = FALSE}
# What's the first argument- elections. The second- column
...
```{r}
# What does this do? # Keep counties in Hawaii AND Delaware
elections_small |> 
...
```{r}
# What did this code do? Creates a row called diff_20 and mutates it onto the data
elections_small |> 
...
```{r}
# What did this code do? mutates the exact number of voters onto our data
elections_small |> 
...
```{r}
# What did this code do? tells us if the republican vote won by TRUE/FALSE
elections_small |> 
...
elections_small |> 
  mutate(dem_20_greater= dem_pct_20 > dem_pct_16) |> 
  head()
```
