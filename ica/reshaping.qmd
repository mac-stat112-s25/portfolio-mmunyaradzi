...
#key ideas- reshape dataset, reassign value e.g quantitaive to categorical, or character to quantitative. Visualize
...
`ggplot(sleep_wide, aes(y = reaction_time, x = day, color = ___))`
...
```{r}
# a <fctr> is a categorical value, previously the 308s, 310s where being taken as quantitative values. Day was recorded as <chr> and we should make it a quantitative value <dbl>
sleep_long <- sleep_wide |>
  pivot_longer(cols = -Subject,
               names_to = "day",
               names_prefix = "day_",
