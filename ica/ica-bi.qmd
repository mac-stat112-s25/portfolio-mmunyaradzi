...
```{r}
ggplot(elections, aes(x = repub_pct_16, color = "red", y = repub_pct_20, color= "blue")) +
    geom_point() +
    theme_classic() +
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
```
...
This is not a good relationship because it does not show any relationship and points are plotted against each other
...
There is a high support that decreases by state from red to purple to blue. 
...
# colors don't match, two of the plots are obscured
...
# enables us to manually enter colors to fill
ggplot(elections, aes(x = repub_pct_20, fill = historical)) +
  geom_density() +
...
# presents plots in separate facets
ggplot(elections, aes(x = repub_pct_20, fill = historical)) +
  geom_density() +
  scale_fill_manual(values = c("blue", "purple", "red")) +
...
# Why is this terrible? *crowds and obscures important info*
...
*show better vizulaization of data*

-   What is one con of density plots relative to boxplots?
*may obscure some details of data distribution esp when comparing multiple groups *
...
```{r eval = FALSE}
# A stacked bar plot
# How are the "historical" and "winner_20" variables mapped to the plot, i.e. what roles do they play? *historical is x-axis while winner is determined by scale of fill*
