...
> imbd_5000_messy.cvs contains data fields from The Movie Database (TMDb) in accordance with their terms of use.
...
```{r}
library(tidyverse)
imdb_messy <- read_csv("~/Desktop/112/portfolio-mmunyaradzi/data/imdb_5000_messy.csv")
```
...
-   *How many* values are missing data? What's the volume of the missingness? 19
...
> Just use the na.rm = True no need to create a new dataset
...
-3756 observations left
-   In what situations might this function be a good idea?
-   In what situations might this function be a bad idea?
 It’s a good idea when we need complete info on every variable for every part of our analysis. It's not a good idea in that many observations may be lost.
...
-   How many movies remain in your new dataset? Hence why this is better than using the dataset from part b? -more movies are kept
